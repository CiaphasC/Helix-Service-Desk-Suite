...
const getLevelColor = (level: Service["level"]) => {
  switch (level) {
    case "critical":
...
export default function CalendarPage() {
  ...
  const servicesForDate = (date: Date) => {
    ...
    return demoServices.filter((service) => service.date === iso)
  }
        ...
        {monthCells.map((cell, index) => {
          ...
          return (
            <button
              ...
              className={`h-24 rounded-lg border transition-all text-left px-3 py-2 group ${
                isActive
  ...
  const dayView = () => {
    ...
    return (
      <Card className="border border-blue-500/20 bg-slate-950/40 backdrop-blur-xl p-6 space-y-4">
        <div className="flex items-center justify-between flex-wrap gap-3">
          ...
          <div className="flex items-center gap-2">
            <select
              className="bg-slate-900/70 border border-slate-700 rounded-lg px-3 py-1.5 text-sm"
        ...
        <div className="space-y-3">
          ...
          {services.map((service) => (
            <div key={service.id} className="flex items-center justify-between border border-slate-800/60 rounded-lg p-3">
              <div>
  ...
  const miniCalendarView = () => (
    <Card className="border border-slate-800/70 bg-slate-950/60 backdrop-blur-xl p-4 space-y-4">
          ...
          return (
            <button
              ...
              className={`h-8 rounded-full transition ${
                isActive ? "bg-primary text-primary-foreground" : "text-foreground/70 hover:bg-slate-800/80"
              }`}
            >
              {cell}
            </button>
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-4 gap-6 p-6">
      <div className="lg:col-span-3 space-y-6">
        <div className="space-y-4">
          <div className="flex flex-wrap items-center gap-4 justify-between">
            ...
            <div className="flex flex-wrap gap-2 bg-slate-900/50 border border-slate-800/60 rounded-xl p-2">
          ...
          <div className="flex flex-wrap gap-3 items-center bg-slate-900/40 border border-slate-800/60 rounded-xl p-4 text-sm">
            <label className="flex flex-col text-muted-foreground">
              Año
              <select
                className="mt-1 bg-slate-950 border border-slate-800 rounded-lg px-3 py-1.5"
                value={currentYear}
      ...
      <div className="hidden lg:block space-y-4">
        {miniCalendarView()}
        <Card className="border border-slate-800/60 bg-slate-950/60 backdrop-blur-xl p-4 space-y-2">
