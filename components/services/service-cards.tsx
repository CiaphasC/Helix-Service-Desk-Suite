...
export function ServiceCards({ services, onEdit, onDelete }: ServiceCardsProps) {
          ...
          <Card className="border border-primary/20 bg-gradient-to-br from-card to-card/50 backdrop-blur-xl p-4 sm:p-6 hover:border-primary/40 transition-all hover:shadow-lg group cursor-pointer h-full flex flex-col">
            <div className="flex-1 mb-4">
              ...
              <div className="flex flex-wrap gap-2 mb-4">
                ...
                >
                  {service.priority === "alta" && "Alta"}
                  {service.priority === "media" && "Media"}
                  {service.priority === "baja" && "Baja"}
                </Badge>
                <Badge
                  variant="outline"
                  className={cn(
                    "text-xs",
                    service.status === "activo" && "border-green-500/50 bg-green-500/10 text-green-500",
